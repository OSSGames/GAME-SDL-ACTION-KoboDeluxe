...
void gfxengine_t::title(const char *win, const char *icon)
{
	...
	_icontitle = icon;
	if(sdl_window)
		SDL_SetWindowTitle(sdl_window, _title ? _title : "");
}
...
int gfxengine_t::show()
{
	...
	sdl_window = SDL_CreateWindow(_title ? _title : "KoboDeluxe",
	...
	if(!screen_surface)
	{
		log_printf(ELOG, "Failed to get window surface!\n");
		return -3;
	}

	/* SDL2: no HW surface or true double buffering. */
	shadow(0);
	doublebuffer(0);
