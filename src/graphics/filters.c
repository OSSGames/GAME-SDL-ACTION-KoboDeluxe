	...
	for(i = 0; i < frames; ++i)
	{
		...
		switch(s_blitmode)
		{
		  ...
		  case S_BLITMODE_OPAQUE:
			...
			break;
		  case S_BLITMODE_COLORKEY:
			SDL_SetColorKey(s->surface, SDL_TRUE,
		...
		if(args->x)
		{
			if(s->surface->format->Amask)
				tweak_ck(s->surface);
			tmp = SDL_ConvertSurfaceFormat(s->surface, SDL_PIXELFORMAT_BGR24, 0);
			if(s->surface->format->Amask)
		...
		{
			if(s->surface->format->Amask)
				tmp = SDL_ConvertSurfaceFormat(s->surface, SDL_PIXELFORMAT_ARGB8888, 0);
			else
				tmp = SDL_ConvertSurfaceFormat(s->surface, SDL_PIXELFORMAT_BGR24, 0);
		}
		if(!tmp)
